...
def test_net(args):
    ...
    test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=args.bs_test,
                                            ...
                                            pin_memory=True)
    base_model, regressor, st_gcn, emg_decoder, _ = builder.model_builder(args)
    # load checkpoints
    ...
    if use_gpu:
        ...
        st_gcn = st_gcn.cuda()
        emg_decoder = emg_decoder.cuda()
        #feature_fuse = feature_fuse.cuda()
        torch.backends.cudnn.benchmark = True

    #  DP
    base_model = nn.DataParallel(base_model)
    regressor = nn.DataParallel(regressor)
    st_gcn = nn.DataParallel(st_gcn)
    emg_decoder = nn.DataParallel(emg_decoder)
    # feature_fuse = nn.DataParallel(feature_fuse)
    test(base_model, regressor, st_gcn, emg_decoder, test_dataloader, group, args)
...
def run_net(args):
    ...
    # optimizer & scheduler
    optimizer, scheduler = builder.build_opti_sche(base_model, regressor, st_gcn, args)
    #optimizer, scheduler = builder.build_opti_sche(regressor,emg_decoder, args)
    # parameter setting
    start_epoch = 0
    ...
    if args.resume:
        start_epoch, epoch_best, rho_best, L2_min, RL2_min = \
            builder.resume_train(base_model, regressor, optimizer, st_gcn, emg_decoder, args)
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        if args.fix_bn:
            ...
            st_gcn.apply(misc.fix_bn)  # fix bn
        ...
        for idx, (data , target) in enumerate(train_dataloader):
            ...
            if args.benchmark == 'MTL':
                ...
                # 这里是样本
                video_1 = data['video'].float().cuda() # N, C, T, H, W
                video_12 = data['video2'].float().cuda()
                skeleton_1 = data['Skeleton'].float().cuda()
                emg_1 = data['EMG'].float().cuda()
                label_1 = data['final_score'].float().reshape(-1,1).cuda()
                # 这里是示例
                video_2 = target['video'].float().cuda()
                video_22 = target['video2'].float().cuda()
                skeleton_2 = target['Skeleton'].float().cuda()
                emg_2 = target['EMG'].float().cuda()
                label_2 = target['final_score'].float().reshape(-1,1).cuda()
                diff = None
            else:
            ...
            helper.network_forward_train(base_model, regressor, st_gcn, emg_decoder, pred_scores, video_1, video_12, skeleton_1, emg_1, label_1, video_2, video_22, skeleton_2, emg_2, label_2, diff, group, mse, nll, optimizer, opti_flag, epoch, idx+1, len(train_dataloader), args)
        ...
        #print('[Training] EPOCH: %d, correlation: %.4f, L2: %.4f, RL2: %.4f, lr1: %.4f, lr2: %.4f'%(epoch, rho, L2, RL2, 0,  optimizer.param_groups[0]['lr']))

        validate(base_model, regressor, st_gcn, emg_decoder, test_dataloader, epoch, optimizer, group, args)
        helper.save_checkpoint(base_model, regressor, st_gcn, emg_decoder, optimizer, epoch, epoch_best, rho_best, L2_min, RL2_min, 'last', args)
        print('[TEST] EPOCH: %d, best correlation: %.6f, best L2: %.6f, best RL2: %.6f'%(epoch, rho_best, L2_min, RL2_min))
        # scheduler lr
        if scheduler is not None:
            scheduler.step()

def validate(base_model, regressor, st_gcn, emg_decoder, test_dataloader, epoch, optimizer, group, args):
    ...
    with torch.no_grad():
        ...
        for batch_idx,  (data , target) in enumerate(test_dataloader, 0):
            ...
            if args.benchmark == 'MTL':
                ...
                # 这里是示例
                video_2_list = [item['video'].float().cuda() for item in target]
                video_22_list = [item['video2'].float().cuda() for item in target]
                skeleton_2_list = [item['Skeleton'].float().cuda() for item in target]
                emg_2_list = [item['EMG'].float().cuda() for item in target]
                label_2_list = [item['final_score'].float().reshape(-1,1).cuda() for item in target]
                diff = None
            else:
                raise NotImplementedError()
            helper.network_forward_test(base_model, regressor, st_gcn, emg_decoder, pred_scores, video_1, video_12, skeleton_1, emg_1, video_2_list, video_22_list, skeleton_2_list, emg_2_list, label_2_list, diff, group, args)
        ...
        if rho > rho_best:
            ...
            helper.save_checkpoint(base_model, regressor, st_gcn, emg_decoder, optimizer, epoch, epoch_best, rho_best, L2_min, RL2_min, 'best', args)

        print('[TEST] EPOCH: %d, correlation: %.6f, L2: %.6f, RL2: %.6f'%(epoch, rho, L2, RL2))

def test(base_model, regressor, st_gcn, emg_decoder, test_dataloader, group, args):
    global use_gpu
    true_scores = []
    pred_scores = []
    base_model.eval()  # set model to eval mode
